...
func NewCodef(code ecode.Code, format string, args ...interface{}) error {
	return &Error{
		stack: callers(),
...
func NewCodeSkip(code ecode.Code, skip int, text ...string) error {
	return &Error{
		stack: callers(skip),
		text:  strings.Join(text, commaSeparatorSpace),
		code:  code,
...
// The parameter `skip` specifies the stack callers skipped amount.
func NewCodeSkipf(code ecode.Code, skip int, format string, args ...interface{}) error {
	return &Error{
		stack: callers(skip),
		text:  fmt.Sprintf(format, args...),
		code:  code,
...
// It returns nil if given `err` is nil.
func WrapCodef(code ecode.Code, err error, format string, args ...interface{}) error {
	if err == nil {
		return nil
	}
	return &Error{
...
// It returns nil if given err is nil.
// The parameter `skip` specifies the stack callers skipped amount.
func WrapCodeSkip(code ecode.Code, skip int, err error, text ...string) error {
	if err == nil {
		return nil
	}
	return &Error{
...
// It returns nil if given err is nil.
// The parameter `skip` specifies the stack callers skipped amount.
func WrapCodeSkipf(code ecode.Code, skip int, err error, format string, args ...interface{}) error {
	if err == nil {
		return nil
	}
	return &Error{
...
// It returns `Nil` if it has no error code neither it does not implement interface Code.
