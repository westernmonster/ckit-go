...
func TestSliceValidationError(t *testing.T) {
	tests := []struct {
		...
		{
			"has nil elements",
			SliceValidationError{errors.New("test error"), nil},
			"[0]: test error",
		},
		{"has zero elements", SliceValidationError{}, ""},
		{
			"has one element",
			SliceValidationError{errors.New("test one error")},
			"[0]: test one error",
		},
		{
			"has two elements",
...
func TestDefaultValidator(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"validate struct failed-1",
			&defaultValidator{},
			exampleStruct{A: "123456789", B: 1},
			true,
		},
		...
		{
			"validate *struct failed-2",
			&defaultValidator{},
			&exampleStruct{A: "12345678", B: 0},
			true,
		},
		{
			"validate *struct passed",
			&defaultValidator{},
			&exampleStruct{A: "12345678", B: 1},
			false,
		},
		{
			"validate []struct failed-1",
