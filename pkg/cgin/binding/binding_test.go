...
type FooBarFileFailStruct struct {
	...
	// for unexport test
	data *multipart.FileHeader `                    binding:"required" form:"data"`
}
...
func TestBindingJSONSlice(t *testing.T) {
	...
	testBodyBindingSlice(
		...
		"/",
		`[{"foo": "123"}]`,
		`[{"foo": "123456789012345678901234567890123"}]`,
	)
}
...
func TestBindingFormForTime(t *testing.T) {
	testFormBindingForTime(
		...
		"/",
		"/",
		"time_foo=2017-11-15&time_bar=&createTime=1562400033000000123&unixTime=1562400033",
		"bar2=foo",
	)
...
func createFormPostRequest(t *testing.T) *http.Request {
	req, err := http.NewRequest(
		...
		bytes.NewBufferString("foo=bar&bar=foo"),
	)
	assert.NoError(t, err)
...
func createFormPostRequestForMap(t *testing.T) *http.Request {
	req, err := http.NewRequest(
		"POST",
		"/?map_foo=getfoo",
		bytes.NewBufferString("map_foo={\"bar\":123}"),
	)
	assert.NoError(t, err)
	req.Header.Set("Content-Type", MIMEPOSTForm)
	return req
...
func testFormBindingForTimeNotUnixFormat(
	t *testing.T,
	method, path, badPath, body, badBody string,
) {
	b := Form
	assert.Equal(t, "form", b.Name())
...
func testBodyBindingDisallowUnknownFields(
	t *testing.T,
	b Binding,
	path, badPath, body, badBody string,
) {
	EnableDecoderDisallowUnknownFields = true
	defer func() {
		EnableDecoderDisallowUnknownFields = false
